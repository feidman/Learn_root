...
```
Rewrite the codes from The guide for beginners
```
...
### 5.4 Multiple histogram
.e.g. `hstack.C`
notes: a nother way to setparameters
```
...
### Fitting Function to Pseudo Data
.e.g: `macro8.C`
notes: 
(1) 对带本底的信号峰进行拟合 gausppar = gaussian + parabola 
(2) TF1 Definition with a function, which takes two variables(double * vars, double* pars). 
Where, the vars mean the number of varialbes,vars[0] represents for x, vars[1] for y; and the pars represents the parameters transferred into the function, just as a inline TF1 Definition.

### Toy Monte Carlo Experiments
.e.g: `macro9.C`
notes:
(1) What's Monte Carlo methods
(2) TCanvas c1, c2; c1->cd() or c2->cd() to access the canvas
...
...
my_tuple->GetEntry(i)
double row_content = my_tuple->GetArgs();
pot=row_content[0]; // **no leaf name**
cur=row_content[1];
temp=row_content[2];
pres=row_content[3];
...
...
e.g.: `write_ntuple_to_file_advanced.C`
notes: **with leaf name**
```
...
Usually n-tuples or trees **span over many files** and it would be difficult to add them manually. ROOT thus kindly provides a helper class n the form of **TChain**.
The constructor of a **TChain** takes the name of the **TTree**(or **TNuple**) as an argument. The files are added with the function **Add(fileName)**, where one can also use wild-cards as shown in the example.
e.g.: `read_ntuple_with_chain.C`
#### 7.2.5 For the advanced user: Processing trees with a selector script

#### 7.2.6 For power-user: Multi-core processing with PROOT lite
